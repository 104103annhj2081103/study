...
void str_prn(HDC hDc,int x, int y, const wchar_t** ptr_ary, int count)
{
	for (int i = 0; i < count; i++)
	{
		TextOut(hDc, 0 + x, (i * 16)+y, ptr_ary[i], wcslen(ptr_ary[i]));
	}
}
    ...
    case WM_PAINT:
        {
			...
			int _count = sizeof(ptr_ary) / sizeof(ptr_ary[0]);

			str_prn(hdc, 128,128, ptr_ary, _count);

			//TextOut(hdc, 0, 0, L"독수리", wcslen(L"독수리"));
